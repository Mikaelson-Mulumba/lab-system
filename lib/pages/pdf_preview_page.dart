...
class _PdfPreviewPageState extends State<PdfPreviewPage> {
  ...
  Future<pw.Document> _buildPdf() async {
    ...
    for (var r in widget.results) {
      final type = r['test_type'] ?? 'Unknown';
      groupedResults.putIfAbsent(type, () => []).add(r);
    }
        ...
        build: (pw.Context context) => [
          ...
          for (var entry in groupedResults.entries) ...[
            ...
            pw.TableHelper.fromTextArray(
              headers: ['Test Name', 'Result', 'Unit', 'Flag', 'Normal Range'],
              data: entry.value
                  .map(
                    (r) => [
                      r['name'] ?? '',
          ...
          pw.Row(
            ...
            children: [
              pw.Column(
                children: [
                  pw.Text(
                    'Lab Technician: ${widget.currentUser}',
              ...
              pw.Column(
                children: [
                  pw.Text(
                    'Approved By: ${widget.currentUser}',
                    style: pw.TextStyle(font: robotoRegular, fontSize: 10),
                  ),
                  pw.SizedBox(height: 20),
                  pw.Text(
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) => [
          ...
          if (widget.payment != null) ...[
            pw.Text(
              'Amount Paid: UGX ${widget.payment!['amount']}',
              style: pw.TextStyle(font: robotoRegular, fontSize: 11),
            ),
            pw.Text(
              'Paid By: ${widget.payment!['paid_by']}',
              style: pw.TextStyle(font: robotoRegular, fontSize: 11),
            ),
            pw.Text(
              'Payment Date: ${DateTime.now().toLocal().toString().split(" ").first}',
              style: pw.TextStyle(font: robotoRegular, fontSize: 11),
            ),
          ],
  ...
  Future<void> _savePdfToDatabase() async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Report saved to database')));
  }
